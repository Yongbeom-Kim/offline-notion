...
import { isInternalDocumentHref, extractDocumentId } from "../../../utils/url";
import { getDocumentMetadata, type DocumentMetadata } from "@/hooks/use-document-store";
...
function findInternalLinksInBlocks(
  blocks: Block[],
  origin: string
): Array<{ blockId: string; linkIndex: number; docId: string; currentText: string }> {
  const links: Array<{ blockId: string; linkIndex: number; docId: string; currentText: string }> = [];
    ...
    for (const block of blocks) {
      if (block.content && Array.isArray(block.content)) {
        for (let i = 0; i < block.content.length; i++) {
          ...
          if (item.type === "link" && item.href && isInternalDocumentHref(item.href, origin)) {
...
function updateLinkText(
  ...
  if (linkItem?.type === "link" && linkItem.content && Array.isArray(linkItem.content)) {
...
export async function updateInternalDocumentLinks(
  ...
  try {
    ...
    const documents = await Promise.all(docIds.map(docId => getDocumentMetadata(docId)));
