...
export const acquireUploadLock = async ({
  ...
  try {
    const res = await apiClient.post(`/upload/g/start/${encodeURIComponent(docId)}`, null, {
      params: { nonce, ttl },
      validateStatus: () => true,
    });
...
export const releaseUploadLock = async ({
  ...
  try {
    const res = await apiClient.post(`/upload/g/end/${encodeURIComponent(docId)}`, null, {
      params: { nonce },
      validateStatus: () => true,
    });
    return res.status === 200;
...
export const checkUploadLock = async (
  ...
  try {
    const res = await apiClient.get(`/upload/g/check/${encodeURIComponent(docId)}`, {
      params: { nonce },
      validateStatus: () => true,
    });
